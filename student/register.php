...
if ($_SERVER['REQUEST_METHOD'] == "POST"){
    ...
    if(empty(trim($_POST["email"]))){
        ...
        if($stmt)
        {
            $stmt->bindParam(":Semail", $param_email);
...
{
    $sql = "INSERT INTO student_login (Semail, Spassword) VALUES (:Semail, :Spassword)";
    $stmt = $conn->prepare($sql);
    if ($stmt)
    {
        $stmt->bindParam(":Semail", $param_email);
        $stmt->bindParam(":Spassword", $param_password);
...
<div class="mainBox" style="height:60vh">
